...
let y0 =
  let open! FDSL.O in
    ...
    ((Formula.FDSL.number (Float.of_int 2)) *. hey1) +
      (Formula.FDSL.number (Float.of_int 3))
let y1 =
  let open! FDSL.O in
    let hey2 = FDSL.unconstrained_param ?init:None "hey2" in
    fun x -> (hey2 * (Formula.FDSL.number (Float.of_int 2))) + x
let y2 =
  let open! FDSL.O in
