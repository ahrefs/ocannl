...
let y1 =
  ...
  let open! TDSL.O in
    fun x ->
      ((+) ~label:["y1"])
        ((( * ) ~label:[]) hey2 (TDSL.number (Float.of_int 2))) x
let y2 =
  let hey3 = TDSL.param ?values:None "hey3" in
  let open! TDSL.O in
    fun x1 -> fun x2 -> ((+) ~label:["y2"]) ((( *. ) ~label:[]) x1 hey3) x2
let a =
  let open! TDSL.O in
    TDSL.ndarray ~label:["a"] ~batch_dims:[] ~input_dims:[3] ~output_dims:
      [2]
      [|(Float.of_int 1);(Float.of_int 2);(Float.of_int 3);(Float.of_int 4);(
        Float.of_int 5);(Float.of_int 6)|]
let b =
  let open! TDSL.O in
    TDSL.ndarray ~label:["b"] ~batch_dims:[2] ~input_dims:[] ~output_dims:
      [2]
