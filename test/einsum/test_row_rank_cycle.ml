(** Regression test for termination of [Row.solve_inequalities] on mutually-growing row variables
    ...
    itself, e.g. {Row_ineq {res = <rho1>; opnd = <rho2>.[a]}, Row_ineq {res = <rho2>;
    opnd = <rho1>.[b]}} with [a], [b] in the trailing flank ([dims], not [beg_dims]) demands
    rank(rho1) >= rank(rho2) + 1 and rank(rho2) >= rank(rho1) + 1 — unsatisfiable. The deficit
    (template) rule used to ping-pong such constraint sets, minting fresh row variables forever
    (the three-variable cycle diverged; the two-variable one happened to collapse into the
    one-step self-reference check). With the transitive rank-cycle check these must raise
    [Shape_error]. *)
