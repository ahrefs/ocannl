(** Pinning test for the closing-order sensitivity of Remark 6.3 (the interleave example) in
    docs/blog/ocannl-formal-core.md.
    ...
    The closing order matters in principle: committing alpha first re-emits 3 <= beta, the
    ...
    terminals (and of the inequalities), pinning which orders the implementation's
    emission-ordered closing sweep survives. *)
