(** Spec-side regression for the beg_dims-on-Row.t refactor.
    ...
    where parser-derived leading axes are now stored in [Row.beg_dims] instead of being prepended to
    [Row.dims]. The pre-fix behavior was: build a row with [bcast = Row_var { v; beg_dims }] and
    then flatten [beg_dims @ dims] into the top-level [dims]. Under the new layout, [beg_dims] is a
    top-level field and the spec parser must place leading axes there directly.
...
let dim_sizes_of dims = List.map dims ~f:(function Row.Dim { d; _ } -> Some d | _ -> None)

let int_opts_equal xs ys =
  List.length xs = List.length ys
  && List.for_all2_exn xs ys ~f:(fun a b ->
      match (a, b) with Some i, Some j -> i = j | None, None -> true | _ -> false)
...
let row_has_structure (row : Row.t) ~expected_beg ~expected_trailing =
  ...
  let opts_to_str os =
    String.concat ~sep:";" (List.map os ~f:(function Some i -> Int.to_string i | None -> "?"))
  in
  if int_opts_equal beg_sizes expected_beg_opts && int_opts_equal dim_sizes expected_trail_opts then
    Ok ()
  else
    Error
      (Printf.sprintf "expected beg_dims=[%s] dims=[%s]; got beg_dims=[%s] dims=[%s]"
...
(* Test 1: parser places leading axes in t.beg_dims, not in t.dims. Spec "3, ..rho.., 7" on the
   output kind: leading [Fixed_index 3], row var rho, trailing [Fixed_index 7]. Under the new layout
   this must build { beg_dims = [Dim 3]; dims = [Dim 7]; bcast = Row_var rho }, NOT the pre-fix {
   beg_dims = []; dims = [Dim 3; Dim 7]; bcast = Row_var rho }. *)
let test_parser_places_leading_in_beg_dims () =
  Stdio.printf "Test: einsum/shape spec parser places leading axes into Row.beg_dims\n";
...
(* Test 2: spec-driven outer-left mismatch raises Shape_error. Build two rows from specs that share
   ...
   disagreement must surface as a Shape_error from the solver — i.e., the spec/parser path produces
   rows where outer-left alignment is enforced by the row inference machinery.

   Concretely: r1 has leading flank [Dim 5; Dim 2] and r2 (open) has leading flank [Dim 2]. Under
   outer-left alignment, r2's first leading axis (Dim 2) aligns against r1's first leading axis (Dim
   5) — incompatible. *)
let test_spec_outer_left_mismatch () =
  ...
  let res =
    {
      Row.beg_dims = [ Row.get_default_dim ~d:5 (); Row.get_default_dim ~d:2 () ];
...
(* Test 3: when the spec parser populates beg_dims, downstream subst preserves them. Spec "3,
   ..rho.., 7" builds row with beg_dims=[Dim 3] and dims=[Dim 7]. Substituting rho := {beg_dims=[];
   dims=[Dim 4]; Broadcastable} via Row_eq must yield a closed row with beg_dims=[Dim 3] and
   dims=[Dim 4; Dim 7]. This exercises the spec → row → substitution chain and would fail if the
   spec parser had reverted to flattening leading axes into dims. *)
let test_spec_substitution_preserves_leading () =
  ...
  let rho =
    match row.bcast with
    | Row.Row_var v -> v
    | Row.Broadcastable -> failwith "Test: spec did not produce a row variable; cannot continue"
  in
  let prov = Row.empty_provenance in
  let value : Row.t =
    { beg_dims = []; dims = [ Row.get_default_dim ~d:4 () ]; bcast = Broadcastable; prov }
  in
  let eq =
    Row.Row_eq
      {
        r1 = { beg_dims = []; dims = []; bcast = Row_var rho; prov };
        r2 = value;
        origin = dummy_origin;
      }
  in
