...
module Ndarray = Arrayjit.Ndarray
...
let%expect_test "uint4x32_to_prec_uniform different precisions" =
  ...
  let test_precision prec prec_name =
    let uniform = O.uint4x32_to_prec_uniform ~target_prec:prec random_bits in
    Train.set_hosted uniform.value;
    let ctx = Train.forward_and_ctx Backend.runner uniform in
    let result = Backend.to_float ctx uniform in
