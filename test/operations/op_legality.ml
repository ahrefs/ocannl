...
let hand_built ~stmts ~tns_on_device ~tns_local =
  ...
  List.iter tns_on_device ~f:(fun tn ->
      Tn.Placements.update plc tn Tn.On_device (Site "49:test-setup"));
  List.iter tns_local ~f:(fun tn -> Tn.Placements.update plc tn Tn.Local (Site "49:test-setup"));
