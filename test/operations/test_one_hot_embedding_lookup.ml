(* gh-ocannl-343 / task-73617488: end-to-end test for the one-hot embedding optimization.
   ...
   The embedding lookup [emb[b,o] = C[o, ids[b]]] is written logically as a one-hot reduction [emb =
   C * (range vocab == ids)]. [rewrite_one_hot_reductions] collapses the vocabulary loop into a
   guarded [Get_dynamic]. task-73617488 makes this fire under the default [virtualize_max_visits =
   1] via a narrow virtualizer exemption for one-hot selector producers.

   Pinned invariants: - Forward equivalence: the logical one-hot embedding equals a direct gather of
   table rows. - Optimization observability: the optimized low-level IR contains [Get_dynamic] and
   no reduction loop over the vocabulary axis. - Out-of-bounds: an index outside [0, vocab) yields a
   zero embedding row (one-hot semantics). - Read tracking: the index tensor is a routine input of
   the optimized gather. - Fallback: an ordinary (non one-hot) contraction is left unchanged (no
   [Get_dynamic]). - Negative (task-73617488): non-one-hot complex pointwise tensors still respect
   [max_visits]. *)
...
let () =
  (* --- Forward equivalence + observability (in-range indices) --- Run first so the forward comp is
     fully assembled and memory modes are fixed, then re-lower the same graph to inspect the
     optimized IR. *)
  ...
  (* Proposal AC: the generated C for the optimized kernel contains a guarded dynamic table read and
     ...
     distinct from the batch/output loop bounds ([<= 2]). Note: [iprec] (the precision of the index
     value expression) comes from the IDs tensor's precision verbatim (default: single/float32,
     exact for integers up to 2^24). For very large vocabularies, callers should use
     double-precision IDs so the value survives to the widened cast without prior float-rounding
     loss. *)
  ...
  let a =
    TDSL.ndarray
      (Array.create ~len:(2 * vocab) 1.)
      ~label:[ "A" ] ~input_dims:[ vocab ] ~output_dims:[ 2 ] ()
  in
  ...
  (* --- Negative-2 (task-73617488): Cmpeq against a non-range input tensor is NOT exempted ---
     [not_hot2[b,k] = (arb_table[k] == ids_neg2[b])] is a Cmpeq, but [arb_table] is a plain input
     (not a range producer), so [is_range_producer = false] and the indirect arm of
  ...
  let arb_table =
    TDSL.ndarray (Array.init vocab ~f:Float.of_int) ~label:[ "arb_table" ] ~output_dims:[ vocab ] ()
  in
  let ids_neg2 =
...
(* --- AC 3: generated-C inspection for the large-index (> INT_MAX) path --- A real gather with
   ...
   double-precision IDs tensor whose first entry 2_200_000_000.0 exceeds INT_MAX (2_147_483_648).
   Two properties must hold: (1) IR level: [Get_dynamic.dyn_value] carries iprec = double — the
   index value is not truncated to float32 (exact only to 2^24 = 16M) before reaching the cast. (2)
   C level: the generated C declares [ids_wide] as [double*] and casts the dynamic index with
   [((uint32_t)(] (default) or [((uint64_t)(] (large_models), not the old [((int)(] which is C
   undefined behaviour for values > INT_MAX. Mutation evidence: (1) fails if
   [Tensor.default_value_prec] is left at single/float32 (ids_wide.value.prec = single → iprec =
   single → inner cast is (float) not (double)); (2) fails if the cast in [c_syntax.ml Get_dynamic
   arm] is reverted to [((int)(]. Note: a 1-element IDs array is always inlined as a [Constant]
   literal by [low_level.scalar_precision], whose [Constant] arm defaults to [single]. We use 2
   elements so [ids_wide] has backing storage and [Get(ids_tn)] carries its declared [double]
   prec. *)
let () =
  ...
  let ids_wide =
    TDSL.ndarray [| 2_200_000_000.0; 0. |] ~label:[ "ids_wide" ] ~batch_dims:[ 2 ] ~output_dims:[]
      ~top_down_prec:false ()
  in
  ...
  (* (2) C-level inspection: emb_wide kernel is [emb_wide_fwd.c]. Mutation evidence: reverting
     [c_syntax.ml Get_dynamic arm] to [((int)(] would change the cast from [((uint32_t)(] to
     [((int)(], flipping both assertions below. *)
  match read_generated_c "emb_wide_fwd" with
  | None -> p "large-index (C): double *ids_wide and wide cast (skipped: non-C backend)" true
  | Some c ->
