(** Regression tests for the beg_dims-on-Row.t refactor.
    ...
    1. Outer-left alignment in solve_row_ineq. 2. s_row_one composes beg_dims faithfully. 3. Closing
    preserves beg_dims via substitution. 4. GLB merge symmetric on both flanks. 5. Monotonicity via
    re-firing. *)
...
(* Test 1: outer-left alignment. res = { beg_dims = [Dim 5; Dim 2]; dims = [Dim 4]; bcast =
   Broadcastable } opnd = { beg_dims = [Dim 2]; dims = [Dim 4]; bcast = Row_var rho } res ⊑ opnd
   must FAIL: outer-left aligns opnd's leading Dim 2 with res's leading Dim 5, which is
   incompatible. *)
let test_1_outer_left_alignment () =
  ...
  let opnd : Row.t = { beg_dims = [ dim 2 ]; dims = [ dim 4 ]; bcast = Row_var rho; prov } in
...
(* Test 2: substitution preserves beg_dims via uniform composition. in_ = { beg_dims = [Dim 3]; dims
   = []; bcast = Row_var rho } value = { beg_dims = []; dims = [Dim 4]; bcast = Broadcastable }
   After solving Row_eq { row_of_var rho; value } and then substituting into in_, the result must be
   { beg_dims = [Dim 3]; dims = [Dim 4]; Broadcastable }. *)
let test_2_substitution_preserves_beg_dims () =
  Stdio.printf "Test 2: s_row_one composes beg_dims (closed value into open row)\n";
  let rho = Row.get_row_var () in
  let in_ : Row.t = { beg_dims = [ dim 3 ]; dims = []; bcast = Row_var rho; prov } in
...
(* Test 3: closing preserves beg_dims. r = { beg_dims = [Dim 7]; dims = []; bcast = Row_var rho }
   Force closure at Stage7 via a Shape_row constraint (which closes row variables in stage 7). After
   substitution, r must close to { beg_dims = [Dim 7]; dims = []; Broadcastable }.

   This test guards the silent-erasure soundness risk: under the new layout, the bare row variable
   closes to empty Broadcastable, and r's beg_dims is preserved through s_row_one's uniform
   composition at substitution time. *)
let test_3_closing_preserves_beg_dims () =
  Stdio.printf "Test 3: closing preserves beg_dims (Row_var v -> Broadcastable)\n";
  let rho = Row.get_row_var () in
  let r : Row.t = { beg_dims = [ dim 7 ]; dims = []; bcast = Row_var rho; prov } in
  let constraints = [ Row.Shape_row (r, dummy_origin) ] in
  let _remaining, env = Row.solve_inequalities ~stage:Stage7 constraints Row.empty_env in
...
(* Test 4: GLB merge symmetric across both flanks. First inequality: res1 = { beg_dims=[Dim 3; Dim
   ...
   3]; dims=[Dim 8; Dim 7]; Broadcastable } opnd = rho_row (same rho) Merged GLB should keep the
   shorter leading flank length (1) and shorter trailing flank length (1). Outer-left Dim 3 matches
   outer-left Dim 3 → Dim 3; outer-right Dim 7 matches outer-right Dim 7 → Dim 7. *)
let test_4_glb_merge_symmetric () =
  ...
  let _remaining, env = Row.solve_inequalities ~stage:Stage1 [ ineq1; ineq2 ] Row.empty_env in
  let _remaining, env =
...
(* Test 4b: leading-flank CONFLICT demotes to unbased Dim 1. Two upper bounds on the same rho with
   different leading-flank values (Dim 3 vs Dim 5) at the same outer-left position. The merge must
   demote to an unbased Dim 1. Trailing flank stays compatible (Dim 7 in both). A regression that
   removed the leading-flank conflict case from join_dim would leave one of the original axes (Dim 3
   or Dim 5) in beg_dims and this assertion would fail. *)
...
(* Test 4c: trailing-flank CONFLICT demotes to unbased Dim 1. Mirror of Test 4b: same leading flank
   (Dim 3), different trailing-flank values (Dim 7 vs Dim 11). The merge must demote the trailing
   flank to an unbased Dim 1. A regression that removed the trailing-flank conflict case from
   join_dim would leave one of the originals (Dim 7 or Dim 11) and this assertion would fail. *)
...
(* Test 5: monotonicity via re-firing (success case). First: solve res ⊑ b with b = rho_row, c =
   {beg_dims=[Dim 4]; dims=[Dim 7]; Broadcastable}. The GLB c is banked for rho. Then: substitute
   rho := {beg_dims=[Dim 4]; dims=[]; bcast=Row_var rho'}. The leading Dim 4 of the substituted row
   aligns outer-left against the banked Dim 4 and the inequality holds. *)
let test_5_monotonicity_via_refiring () =
  ...
  let c : Row.t = { beg_dims = [ dim 4 ]; dims = [ dim 7 ]; bcast = Broadcastable; prov } in
  let ineq = Row.Row_ineq { res = c; opnd = b; origin = dummy_origin } in
  let _remaining, env = Row.solve_inequalities ~stage:Stage1 [ ineq ] Row.empty_env in
  let subst : Row.t = { beg_dims = [ dim 4 ]; dims = []; bcast = Row_var rho'; prov } in
  let rho_row : Row.t = { beg_dims = []; dims = []; bcast = Row_var rho; prov } in
...
(* Test 5b: monotonicity via re-firing (negative mutation). Same setup as Test 5 but the substituted
   ...
   After substitution, the banked GLB must still be enforced — re-firing via the substituted row
   should expose the conflict either at substitution time or when the row variable is closed against
   its GLB. We attempt to close rho' (the new row variable) and check that subst_row of rho's row
   reflects either: (a) an explicit Shape_error during solving (the banked fact is retained and
   rejects the bad substitution), or (b) a substituted result that has not silently lost the GLB
   constraint — i.e., the closed row must contain the GLB's Dim 4 somewhere; a regression that
   dropped the banked fact would produce a clean Dim 5 with no trace of Dim 4.
...
let test_5b_monotonicity_negative_mutation () =
  ...
  let ineq = Row.Row_ineq { res = c; opnd = b; origin = dummy_origin } in
  let _remaining, env = Row.solve_inequalities ~stage:Stage1 [ ineq ] Row.empty_env in
  (* Mutation: substitute rho with a row whose leading Dim 5 CONFLICTS with the banked GLB's leading
     Dim 4. *)
  let subst : Row.t = { beg_dims = [ dim 5 ]; dims = []; bcast = Row_var rho'; prov } in
  let rho_row : Row.t = { beg_dims = []; dims = []; bcast = Row_var rho; prov } in
  let eq = Row.Row_eq { r1 = rho_row; r2 = subst; origin = dummy_origin } in
  let raised_during_subst, env_after_subst =
    try
      let _remaining, env = Row.solve_inequalities ~stage:Stage1 [ eq ] env in
      (false, env)
  ...
  if raised_during_subst then
    ...
    in
    match closed with
    | None -> Stdio.printf "  PASS: banked fact rejected the conflict at close time\n"
    | Some result ->
