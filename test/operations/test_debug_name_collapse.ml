...
let () =
  ...
  (* AC5: filter-before-collapse — non-alphanum_ component "-" is removed first,
     turning originally non-consecutive foo's into a consecutive run of 3. *)
  check "filter-before-collapse: foo,-,foo,foo -> foo3" "foo3"
    [ "foo"; "-"; "foo"; "foo" ];
  (* AC5: grad-strip-before-collapse — leading "grad" is peeled off (sets is_grad),
     leaving ["grad";"grad"] which collapses to "grad2"; then .grad is appended. *)
