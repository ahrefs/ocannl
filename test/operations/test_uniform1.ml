...
let%expect_test "uniform1 basic test" =
  ...
  let uniform1_floats = 
    O.threefry4x32 
      ...
      range_tensor 
    |> O.uint4x32_to_prec_uniform1 in
  ...
  ignore (Ocannl.Train.forward_once (module Backend) uniform1_floats);
  ...
  [%expect
    {|
    First 5 uniform1 random values:
      [0]: 0.1881
      [1]: 0.07396
      [2]: 0.494
      [3]: 0.1023
    ...
    Vectorized version produces 40 values from 10 uint4x32 inputs
    Non-vectorized version produces 10 values from 10 uint4x32 inputs
    |}]

let%expect_test "uniform_at1 test" =
  let module Backend = (val Backends.fresh_backend ()) in
  let module TDSL = Ocannl.Operation.TDSL in
  let module O = TDSL.O in
  ...
  [%expect
    {|
    uniform_at1 random values with counter:
      [0]: 0.9576
      [1]: 0.5901
      [2]: 0.4797
      [3]: 0.4178
...
let%expect_test "uniform1 shape preservation" =
  let module Backend = (val Backends.fresh_backend ()) in
  let module TDSL = Ocannl.Operation.TDSL in
  let module O = TDSL.O in
  ...
  ignore (Ocannl.Train.forward_once (module Backend) uniform1_tensor);
  ...
  ignore (Ocannl.Train.forward_once (module Backend) uniform_vec);
  ...
  [%expect
    {|
    Input size: 24 elements
    uniform1 output size: 24 elements
    Shape preserved (1:1 mapping): true
    
    Vectorized uniform output size: 96 elements
    Vectorized expands by factor of 4
    |}]