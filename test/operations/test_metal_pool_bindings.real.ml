(* Regression test for gh-ocannl-344 AC 4: the Metal backend reaches a kernel's materialized tensor
   nodes through a small fixed set of bound pool buffers + a slot table, not one argument binding
   per node. This sums 40 distinct ndarray-backed constants in one expression, so the forward kernel
   ...
   correct result. Under the old one-buffer-per-tnode binding this kernel would issue 40+
