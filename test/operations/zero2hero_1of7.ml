...
let%expect_test "Graph drawing recompile" =
  ...
  let ctx = Train.forward_once ctx f_nd in
...
let%expect_test "Graph drawing fetch" =
  ...
  let%op f5 = f 5 in
  Train.every_non_literal_materialized f5;
  let ctx = Train.forward_once ctx f5 in
...
let%expect_test "Simple gradients virtual" =
  ...
  [%expect
    {|
                   ...
                   #13 grad_*._l unknown
                   <not-in-context>
             #8 +_d unknown               │#10 f non-emb
             <not-in-context>             │ -2.00
             #9 grad_+_d unknown          │#11 grad_f
             <not-in-context>             │ 4.00
      #4 *._e unknown        │#6 c non-emb│
      <not-in-context>       │ 1.00e+1    │
      #5 grad_*._e unknown   │#7 grad_c   │
      <not-in-context>       │ -2.00      │
    #0 a non-emb│#2 b non-emb│            │
     2.00       │ -3.00      │            │
    #1 grad_a   │#3 grad_b   │            │
...
let%expect_test "2D neuron virtual" =
  ...
  [%expect
    {|
              ...
              #9 grad_+_v unknown
              <not-in-context>
         #6 * unknown          │#4 b non-emb
         <not-in-context>      │ 6.70
         #7 grad_* unknown     │#5 grad_b
         <not-in-context>      │ 1.00
    #0 w non-emb │#2 x non-emb │
     -3.00  1.00 │ 2.00  0.00  │
    #1 grad_w    │#3 grad_x    │
