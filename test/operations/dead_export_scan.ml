(* gh-ocannl-806: a top-level helper in a module without an [.mli] is an exported value, even when
   ...
   counts every value because it re-exports the interface. Values generated by [of_sexp], [sexp],
   [compare], and [equal] derivings are exports too; the matching expression extensions count as
   references to the same generated names. The [sexp_of] converters, derived or hand-written, are
   excluded by policy (the support module's header says why): they are the debugging and
   observability entry point, and [ppx_minidebug]'s typed log annotations consume them invisibly to
   a source-level census. Other PPX-generated values and values brought into the defining module by
   [include] are outside this source-level census.
