(** MNIST digit classification using LeNet-style CNN.

    Demonstrates: loading real-world data from [Dataprep], [int8] to [float] conversion, CNN
    training with cross-entropy loss, and held-out test evaluation.
    ...
    training that targets the issue acceptance criteria (>95% test accuracy):
...
let () =
  ...
  let (train_images_raw, train_labels_raw), (test_images_raw, test_labels_raw) =
    load_quietly (fun () -> Dataprep.Mnist.load ~fashion_mnist:false)
  in

  let n_batches = num_train / batch_size in
  let n_test_batches = num_test / batch_size in

  (* Convert int8 images to float32 [N; 28; 28; 1], normalize to [0, 1] *)
  let train_images_f32 = Conv_data.mnist_images_to_float32 train_images_raw in
  ...
  let test_images_f32 = Conv_data.mnist_images_to_float32 test_images_raw in
