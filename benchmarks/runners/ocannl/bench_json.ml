...
(** The result line [orchestrate.py] parses, as a string without its trailing newline.
    ...
    [peak_memory] is the cell's peak device footprint over the timed steps and the name of the
    counter it was read from, or [None] for a cell that measured none -- which reaches the report as
    a dash rather than a zero (gh-ocannl-1006). The counter is NAMED on the wire rather than left to
    be inferred from the framework and backend columns, because the available counters are not all
    the same quantity: an OCANNL row and a [torch.cuda.max_memory_allocated] row are both requested
    bytes off an allocator's high-water mark and compare honestly, whereas a current gauge sampled
    at step boundaries ([torch.mps], tinygrad) is a lower bound on the same window. A reader who
    cannot see which one a row carries would compare them as though they were one column. *)
let result_line ~backend ~variant ~precision ~profile ~regime_knobs ~workload ~compile_s ~searched
  ...
  in
  let peak_bytes_field, peak_source_field =
    match peak_memory with
    | None -> ("null", "null")
    | Some (bytes, source) -> (Int.to_string bytes, Printf.sprintf {|"%s"|} (string source))
  in
  Printf.sprintf
    {|{"framework":"ocannl","backend":"%s","variant":"%s","precision":"%s","profile":%s,"regime_knobs":%s,"workload":"%s","compile_s":%s,"searched":%b,%s%s"step_ms":{"p10":%s,"p50":%s,"p90":%s},"queued_step_ms":%s,"timed_steps":%d,"peak_memory_bytes":%s,"peak_memory_source":%s,"losses":[%s]}|}
    ...
    (num p90) (num queued_ms) timed_steps peak_bytes_field peak_source_field (nums ~prec:9 losses)
