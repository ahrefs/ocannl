...
crossover is where the arithmetic said: on the CPU backend it is neutral at seq 128 and **28%
faster at seq 1024**; on Metal it costs 3–8% at seq 128–512 (one row per thread with a serial key
loop under-occupies the GPU while the composed form's `seq^2` kernels are still cheap) and is
**18% faster at seq 1024**, where the composed form's two `[seq, seq]` intermediates dominate.
Leaving the score matrix to the recompute cap (stored, the default) is the right default on both
backends: recomputing it never beats storing it by more than the run-to-run spread and loses by
up to 16%. Every arm reproduces the composed losses to 1.4e-7 relative.

Hardware: Apple M4 Max (40-core GPU), macOS 26.6.2. Tree: this branch at `a7eca5919` (staging
master `b6ab4b939` plus the rewrite). Every number is `bench_gpt` on the fixture named, untuned
default pipeline (no `BENCH_TUNE`, no `model_default_schedule`), f32, one process per cell, arms
dispatched as command-line flags:
...
Repeats: cc twice (arm order forward, then reversed), Metal three times (forward, forward,
reversed), so an order effect would show as a split between the repeats of one cell. Parity
...
|---|---|---|---|---|---|
| `gpt2_mini` (8 x 128) | composed | 2109, 2320 | 2214.6 | 1.000x | up to 1.128x |
| `gpt2_mini` (8 x 128) | online, scores stored | 2058, 2345 | 2201.6 | 0.994x | up to 1.049x |
| `gpt2_mini` (8 x 128) | online, scores recomputed | 2318, 2345 | 2331.3 | **1.053x** | up to 1.105x |
| `gpt2_mini_s512` (2 x 512) | composed | 2559, 2624 | 2591.4 | 1.000x | up to 1.095x |
| `gpt2_mini_s512` (2 x 512) | online, scores stored | 2396, 2428 | 2412.1 | **0.931x** | up to 1.259x |
| `gpt2_mini_s512` (2 x 512) | online, scores recomputed | 2342, 2339 | 2340.5 | **0.903x** | up to 1.057x |
| `gpt2_mini_s1024` (1 x 1024) | composed | 2459, 2338 | 2398.4 | 1.000x | up to 1.048x |
| `gpt2_mini_s1024` (1 x 1024) | online, scores stored | 1760, 1689 | 1724.8 | **0.719x** | up to 1.035x |
| `gpt2_mini_s1024` (1 x 1024) | online, scores recomputed | 2087, 1978 | 2032.3 | **0.847x** | up to 1.025x |

### Metal

| fixture (batch x seq) | arm | p50 per repeat (ms) | median p50 | vs composed | p10..p90 spread |
|---|---|---|---|---|---|
| `gpt2_mini` (8 x 128) | composed | 125, 125, 127 | 125.2 | 1.000x | up to 1.020x |
| `gpt2_mini` (8 x 128) | online, scores stored | 129, 129, 133 | 129.2 | 1.032x | up to 1.017x |
| `gpt2_mini` (8 x 128) | online, scores recomputed | 131, 131, 132 | 130.8 | 1.045x | up to 1.033x |
| `gpt2_mini_s512` (2 x 512) | composed | 437, 437, 437 | 437.0 | 1.000x | up to 1.011x |
| `gpt2_mini_s512` (2 x 512) | online, scores stored | 473, 474, 528 | 473.6 | **1.084x** | up to 1.216x |
| `gpt2_mini_s512` (2 x 512) | online, scores recomputed | 507, 505, 518 | 506.6 | **1.159x** | up to 1.036x |
| `gpt2_mini_s1024` (1 x 1024) | composed | 53.9, 54.7, 54.2 | 54.2 | 1.000x | up to 1.013x |
| `gpt2_mini_s1024` (1 x 1024) | online, scores stored | 44.7, 44.5, 44.6 | 44.6 | **0.823x** | up to 1.020x |
| `gpt2_mini_s1024` (1 x 1024) | online, scores recomputed | 49.2, 46.6, 46.5 | 46.6 | **0.859x** | up to 1.089x |

Bold marks a ratio outside 5% of the composed arm. The cc `gpt2_mini` cells' two repeats differ by
10% between the forward-order and reversed-order runs in every arm alike (the box was busier
during the second pass), so the 0.994x there is "neutral within the spread", not a win.
...
- **cc.** Attention is a third of the step's flops at seq 1024 on this fixture (1024 tokens,
  d_head 32) and the `[seq, seq]` traffic is what the rewrite removes, so the 28% at seq 1024
  is most of the attention bin. Storing the scores beats recomputing them by 15% there: the
  recompute doubles the `q * k^T` dot products, and on the CPU those are the expensive part once
  the buffers are gone. At seq 512 the two are within the spread of each other.
- **Metal.** The rewritten attention runs one row per thread with a serial loop over the keys
  ...
  loses 18%. Recomputing the scores costs another 4–8% on top of storing them at every length:
...
- **The autotuned variants.** Every cell is the untuned default pipeline. The rewritten
  attention's value pass is an ordinary loop nest the schedule search can tile; the scan's
  enclosing row loops keep their full menu; the scan body itself does not.
