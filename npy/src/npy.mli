...
(** [write ?header_len bigarray filename] writes a npy file [filename] with the content of
    [bigarray]. [header_len] can be used to override the npy header length. This is only useful for
    testing. *)
...
(** [read_mmap filename ~shared] returns a packed bigarray mmaped to the content of [filename]. If
    [shared] is [true] modifications made to the array are reflected to the file. *)
...
module Npz : sig
  ...
  val restore : ?suffix:string -> in_file -> string -> ('a, 'b, 'c) Bigarray.Genarray.t -> unit
...
(** [to_bigarray layout kind packed_array] returns [Some a] with [a] a [Bigarray.Genarray.t] if the
    layout and the kind of [packed_array] were equal to the [layout] and [kind] arguments.
    Otherwise, [to_bigarray] returns [None] *)
