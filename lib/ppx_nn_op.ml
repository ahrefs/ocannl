...
let ndarray_op ?axis_labels ?label expr =
  ...
  let op =
    ...
    | None, None -> [%expr Formula.CLI.ndarray]
    | Some axis_labels, None -> [%expr Formula.CLI.ndarray ?axis_labels:[%e axis_labels]]
    | None, Some label -> [%expr Formula.CLI.ndarray ?label:[%e label]]
    | Some axis_labels, Some label ->
      [%expr Formula.CLI.ndarray ?axis_labels:[%e axis_labels] ?label:[%e label]] in
  [%expr
...
let make_vb ?init ~loc ~str_loc ~ident string =
  ...
  let v = [%expr CLI.unconstrained_param ?init:[%e init] [%e string]] in
...
let rec translate expr =
  ...
  | { pexp_desc = Pexp_constant (Pconst_float _); _ } ->
    no_vbs, [%expr Formula.CLI.number [%e expr]]

  | { pexp_desc = Pexp_constant (Pconst_integer _); _ } ->
    no_vbs, [%expr Formula.CLI.number (Float.of_int [%e expr])]
  ...
  | [%expr [%e? { pexp_desc = Pexp_constant (Pconst_char ch); pexp_loc; _ }]
    ...
    no_vbs, [%expr Formula.CLI.number ~axis_label:[%e axis] [%e f]]

  | [%expr [%e? { pexp_desc = Pexp_constant (Pconst_char ch); pexp_loc; _ }]
      [%e? { pexp_desc = Pexp_constant (Pconst_integer _); _ } as i]] ->
        let axis = Ast_helper.Exp.constant ~loc:pexp_loc
        (Pconst_string (String.of_char ch, pexp_loc, None)) in
    no_vbs, [%expr Formula.CLI.number ~axis_label:[%e axis] (Float.of_int [%e i])]
  ...
  | [%expr [%e? expr1] **.
    ...
    vbs, [%expr CLI.O.( **. ) [%e e1] [%e f]]

  | [%expr [%e? expr1] **.
      [%e? { pexp_desc = Pexp_constant (Pconst_integer _); _ } as i]] ->
    let vbs, e1 = translate expr1 in
    vbs, [%expr CLI.O.( **. ) [%e e1] (Float.of_int [%e i])]
    ...
    (* We are at the %ocannl annotation level: do not tranlsate the body. *)
     let vbss, bindings = List.unzip @@ List.map bindings
      ~f:(fun vb ->
        ...
        vbs, {vb with pvb_expr=[%expr let open! CLI.O in [%e v]]}) in
...
let translate_str ({pstr_desc; pstr_loc=loc; _} as str) =
  ...
  | Pstr_value (recf, bindings) ->
    let f vb =
      ...
      {vb with pvb_expr=[%expr let open! CLI.O in [%e v]]} in
