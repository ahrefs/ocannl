...
module Env : sig
  ...
  val unify_dim : dim * dim -> t -> t
  val update_dim : is_complete:bool -> dim_var -> ?cur:dim -> ?subr:dim -> t -> t
  val unify_row : dims * dims -> t -> t
  val update_row : is_complete:bool -> int -> ?cur:dims -> ?subr:dims -> t -> t
  val apply_constraint : dims -> t -> t
  val add_dim_ineq : is_complete:bool -> cur:dim -> subr:dim -> t -> t
  val add_row_ineq : is_complete:bool -> cur:dims -> subr:dims -> t -> t
  val empty_env : t
end = struct
  type 'a entry = { cur : 'a list; subr : 'a list; solved : 'a option } [@@deriving sexp]
...
type inequality =
  ...
  | Row_eq of { r1 : dims; r2 : dims }
  | Dim_ineq of { cur : dim; subr : dim }
  | Row_ineq of { cur : dims; subr : dims }
[@@deriving compare, equal, sexp]

let solve_inequalities ~is_complete ineqs env =
  let f env = function
    | Dim_eq { d1; d2 } -> Env.unify_dim (d1, d2) env
    | Row_eq { r1; r2 } -> Env.unify_row (r1, r2) env
    | Dim_ineq { cur; subr } -> Env.add_dim_ineq ~is_complete ~cur ~subr env
    | Row_ineq { cur; subr } -> Env.add_row_ineq ~is_complete ~cur ~subr env
  in
...
let get_proj_equations inequalities proj_axis_env env =
  let to_proj : dim -> proj = function
    ...
    | d -> (
        ...
        | Dim { proj_id = Some proj_id; d; _ } -> Proj { proj_id; d }
...
let get_slot_proj proj_env = function
  | Dim { proj_id = Some p; _ } as d -> (
      let repr, _ = Utils.union_find ~equal:Int.equal proj_env.proj_classes ~key:p ~rank:0 in
      match Map.find proj_env.proj_to_index repr with
      | Some i -> i
      | None -> raise @@ Shape_error ("projection_of_solved_dims: unknown projection", [ Dim_mismatch [ d ] ])
      )
  | Dim { proj_id = None; _ } as d ->
      raise @@ Shape_error ("projection_of_solved_dims: no projection", [ Dim_mismatch [ d ] ])
  | Var _ as d ->
      raise @@ Shape_error ("projection_of_solved_dims: still not fully inferred", [ Dim_mismatch [ d ] ])
  ...
  let project rhs =
    ...
    let get_product_proj = function
      | Dim { d; proj_id = Some p; _ } ->
          let repr = proj_repr p in
          if iterated d && Map.mem proj_env.product_dim repr then Some (repr, d) else None
    ...
    let product_iterators =
      Array.of_list_map all_product_projs ~f:(fun (p, _) ->
          match Map.find_exn proj_env.proj_to_index p with Iterator s -> s | _ -> assert false)
    in
    let f (sh : t) : axis_index array = Array.of_list_map (dims_of sh) ~f:(get_slot_proj proj_env) in
    {
      product_space;
      lhs_dims;
      rhs_dims;
...
let make ?batch_dims ?input_dims ?output_dims ?batch_axes ?input_axes ?output_axes
  ...
  | Not_constrained -> ()
  | Input_equals_output -> (
      try state := Env.unify_row (input, output) !state
