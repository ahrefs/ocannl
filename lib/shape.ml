...
let shift_axes_of_kind kind (axes : 'a axis_map) ~f =
  ...
  @@ List.filter_map (Map.to_alist axes) ~f:(fun (({ in_axes; from_end }, v) as kv) ->
...
let deduce_dims from : deduce_dims -> dims = function
  ...
  | Input_output_scale sc -> (
      ...
      | Given dims | Fixed dims | Inferred dims ->
          Inferred
            (List.map dims ~f:(fun d -> if d = 1 then 1 else Float.(iround_exn ~dir:`Up @@ (sc * of_int d)))))
...
let rec propagate_shapes (update : update_step) =
  ...
  let einsum_one_dim_opt debug_spec debug1 debug2 label terms =
    List.fold terms ~init:(false, None) ~f:(fun ((is_fixed, dim) as accu) (_axis, dims) ->
        ...
        | Some dim1, (Inferred [ dim2 ] | Given [ dim2 ] | Fixed [ dim2 ]) ->
            raise
            @@ Shape_error
                 ( ("Dimension mismatch " ^ Int.to_string_hum dim1 ^ " vs. " ^ Int.to_string_hum dim2
  ...
  | Broadcast (Dynamic_index { over_kind; from_left; other_axes_pointwise }, sh1, sh2) ->
      ...
      let reduced_sh2 =
        { reduced_sh2 with axis_labels = shift_axes_of_kind AxisKey.Output sh2.axis_labels ~f:(( - ) 1) }
      in
      (if from_left then (
       ...
       let reduced_sh1 =
         { reduced_sh1 with axis_labels = shift_axes_of_kind over_kind sh1.axis_labels ~f:drop_left }
       in
       ...
       | None ->
           ...
           let sh1_axis_labels =
             Map.fold sh1.axis_labels ~init:sh1_axis_labels
               ~f:(fun ~key:({ in_axes; from_end } as key) ~data labels ->
                 if AxisKey.equal_kind over_kind in_axes && from_end > sh1_size - subs then
           ...
           let restored_dims over_dims =
             map_dims over_dims ~f:(fun d -> List.concat [ added_dims; List.take d subs; updated_dims ])
           in
       ...
       | Some extended_sh ->
        ...
        let reduced_dims over_dims = map_dims over_dims ~f:(fun d -> List.take d @@ (List.length d - subs)) in
        let reduced_sh1 = map_over_kind over_kind ~f:reduced_dims sh1 in
        let drop_right from_end = from_end - subs in
        let reduced_sh1 =
          { reduced_sh1 with axis_labels = shift_axes_of_kind over_kind sh1.axis_labels ~f:drop_right }
        in
        let extended_sh, logic =
          if other_axes_pointwise then (None, Broadcast (Pointwise_bin, reduced_sh1, reduced_sh2))
          else
        ...
        | None ->
            ...
            let sh1_axis_labels =
              Map.fold sh1.axis_labels ~init:sh1_axis_labels
                ~f:(fun ~key:({ in_axes; from_end } as key) ~data labels ->
                  if AxisKey.equal_kind over_kind in_axes && from_end <= subs then
                    Map.add_exn labels ~key ~data
                  else labels)
            in
            sh1.axis_labels <- sh1_axis_labels;
            let inferred_dims = dims_of_kind over_kind reduced_sh1 |> list_of_dims in
            let restored_dims over_dims =
              map_dims over_dims ~f:(fun d -> inferred_dims @ List.drop d @@ (List.length d - subs))
            in
            if not @@ is_given_or_fixed @@ dims_of_kind over_kind sh1 then
              update_kind over_kind sh1 ~f:restored_dims
        | Some extended_sh ->
...
let rec derive_projections (shapes : update_step) : projections =
  ...
  let broadcast_dims (sh1_dims : dims) (sh2_dims : dims) : int list =
    let rec broad_back_dims accu = function
      ...
      | d1 :: dims1, d2 :: dims2 -> broad_back_dims (max d1 d2 :: accu) (dims1, dims2)
    in
    match (sh1_dims, sh2_dims) with
    | Unknown, Unknown -> []
    | (Inferred dims | Given dims | Fixed dims), Unknown | Unknown, (Inferred dims | Given dims | Fixed dims)
  ...
  | Transpose (Permute spec, sh) ->
      ...
      let rhs_batch : symbol axis_index list =
        map_with_dims sh.batch b_rhs ~f:(fun d it -> if d = 1 then Fixed_idx 0 else it)
      in
      let rhs_input : symbol axis_index list =
        map_with_dims sh.input i_rhs ~f:(fun d it -> if d = 1 then Fixed_idx 0 else it)
      in
      let rhs_output : symbol axis_index list =
        map_with_dims sh.output o_rhs ~f:(fun d it -> if d = 1 then Fixed_idx 0 else it)
      in
  ...
  | Broadcast (Dynamic_index { over_kind; from_left; other_axes_pointwise }, sh1, sh2) ->
      ...
      let reduced_sh1, target_dims, logic =
        if from_left then
          ...
          let sh1_size = List.length @@ list_of_dims @@ dims_of_kind over_kind sh1 in
          let target_dims =
            dims_of_kind over_kind sh1 |> list_of_dims |> Fn.flip List.take subs |> Array.of_list
          in
          let drop_left from_end = if from_end > sh1_size - subs then -1 else from_end in
          let reduced_sh1 =
            { reduced_sh1 with axis_labels = shift_axes_of_kind over_kind sh1.axis_labels ~f:drop_left }
...
let derive_index iterator_symbols (projection : symbolic_axis array) : symbol array -> symbolic_axis array =
  let sym_to_i =
