...
let%track_sexp propagate_shapes (update_step : update_step) : unit =
  ...
  let ineqs', env = Row.solve_inequalities ~stage:Row.Stage1 ~active_update_rows:[] ineqs !state in
...
let%track_sexp finish_inference (() : unit) : unit =
  let active_update_rows = List.concat_map ~f:all_rows !active_update_steps in
  let unsolved, env = Row.solve_inequalities ~stage:Stage2 !active_constraints ~active_update_rows !state in
  active_constraints := [];
  let unsolved, env = Row.solve_inequalities ~stage:Stage3 unsolved ~active_update_rows env in
...
let%track_sexp derive_projections (update_step : update_step) : Idx.projections =
  ...
  let ineqs', local_env = Row.solve_inequalities ~stage:Stage2 ~active_update_rows:[] ineqs Row.empty_env in
