...
let (* %debug_sexp *) propagate_shapes ?remaining_constraints (update_step : update_step) : unit =
  ...
  let ineqs', env = Row.solve_inequalities ineqs !state in
...
let close_update_step env ~remaining_constraints:eqs (update_step : update_step) =
  let f sh =
...
let (* %debug_sexp *) finish_inference (() : unit) : unit =
  ...
  let unsolved, env = Row.solve_inequalities !remaining_constraints !state in
  let remaining_constraints = ref unsolved in
  List.iter !second_stage_inference ~f:(close_update_step env ~remaining_constraints);
  let unsolved, env = Row.solve_inequalities !remaining_constraints env in
...
let (* %debug_sexp *) derive_projections (update_step : update_step) : Idx.projections =
  ...
  let ineqs', local_env = Row.solve_inequalities ineqs Row.empty_env in
