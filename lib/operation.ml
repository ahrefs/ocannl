...
let rec pointpow ?desc_label ~is_diff p m1 : Tensor.t =
  let module NFDSL = struct
  ...
  in
  let%nn_cd grad_body =
    if not is_diff then fun ~n:_ ~n1:_ ~n2:_ ~projections:_ -> Noop
  ...
  in
  Tensor.binop ?desc_label ~compose_op:Pointwise_bin ~op_label:"**." ~op_body ~grad_body ~is_diff m1 p_f
...
let stop_broadcast m = Shape.set_dims_type m.Tensor.shape Shape.fixed

(** [identity] introduces a new node, which is an identity in both the forward and backward pass. *)
let identity ?desc_label ~is_diff m =
  let grad_body ~(n : Code.node) ~(n1 : Code.node) = assign_op (Code.CDSL.data_of_node Grad) ~n:n1 ~n1:n in
  let op_body = assign_op @@ Code.CDSL.data_of_node Value in
  Tensor.(
    unop ?desc_label ~init_shape:m.shape ~transpose_op:Pointwise_un ~op_label:"=" ~op_body ~grad_body ~is_diff)
