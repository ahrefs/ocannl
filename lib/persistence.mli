...
(** [save ~appending t_set path] writes tensor data to a checkpoint file.
    ...
    Raises if any tnode in [t_set] has no hosted array (e.g., virtual or local tnodes). Calls
    [Tnode.do_read] on each tnode to sync device-to-host before saving. *)

val load : ?prefix_namespace:string -> string -> Ocannl_tensor.Tensor.tn_set
(** [load ?prefix_namespace path] reads tensors from a checkpoint file, creates new tnodes,
    registers them, and returns the resulting set.
...
(** [restore t_set path] updates existing tensor buffers from a checkpoint file.
    ...
    For each tnode in [t_set], finds its data in the file by ID and overwrites the hosted buffer.
    After restoring, clears [prepare_read] (to prevent stale device-to-host transfers) and calls
    [do_write] (to mark all devices as stale so the next device access re-transfers from host).
