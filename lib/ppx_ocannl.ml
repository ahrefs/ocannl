...
let let_opt ~loc vbs expr =
  if List.is_empty vbs then expr
  else Ast_helper.Exp.let_ ~loc Nonrecursive vbs expr
...
let rec translate expr =
  ...
  | { pexp_desc = Pexp_construct ({txt=Lident "::"; _}, _); _ } ->
    [], ndarray_constant expr
    
  | [%expr [%e? expr1] [%e? expr2] [%e? expr3] ] ->
    let vbs1, e1 = translate expr1 in
    let vbs2, e2 = translate expr2 in
    let vbs3, e3 = translate expr3 in
    vbs1 @ vbs2 @ vbs3, [%expr Network.apply (Network.apply [%e e1] [%e e2]) [%e e3]]

  | [%expr [%e? expr1] [%e? expr2] ] ->
    let vbs1, e1 = translate expr1 in
    let vbs2, e2 = translate expr2 in
    vbs1 @ vbs2, [%expr Network.apply [%e e1] [%e e2]]
  ...
  | [%expr [%e? expr1] ; [%e? expr2] ] ->
    let vbs1, e1 = translate expr1 in
    let vbs2, e2 = translate expr2 in
    vbs1 @ vbs2, [%expr [%e e1] ; [%e e2]]

  | [%expr if [%e? expr1] then [%e? expr2] else [%e? expr3]] ->
    let vbs2, e2 = translate expr2 in
    let vbs3, e3 = translate expr3 in
    vbs2 @ vbs3, [%expr if [%e expr1] then [%e e2] else [%e e3]]
  ...
  | { pexp_desc = Pexp_match (expr1, cases); _ } ->
    let vbss, cases =
     ...
     List.concat vbss, { expr with pexp_desc = Pexp_match (expr1, cases) }
  ...
  | { pexp_desc = Pexp_let (recflag, bindings, body); _ } ->
     ...
     let all_bindings = List.concat (vbss1 @ [bindings; vbs2]) in
...
let expr_expander ~loc ~path:_ payload =
  ...
  | { pexp_desc = Pexp_let (recflag, bindings, body); _ } ->
    (* We are at the %ocannl annotation level: do not tranlsate the body. *)
     ...
     let_opt ~loc (List.concat vbss) expr
  | expr ->
