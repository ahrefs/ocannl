...
let rec collect_fun accu = function
  ...
  | [%expr ([%e? body] : [%t? {ptyp_desc=Ptyp_constr (t_lident_loc, []); _} as typ ] ) ] ->
...
let debug_fun callback bind descr_loc t_lident_loc_opt1 exp =
  ...
  let arg_logs = List.filter_map args ~f:(function
      | [%pat? ([%p? {ppat_desc=Ppat_var descr_loc; _} as pat ] :
                  [%t? {ptyp_desc=Ptyp_constr (t_lident_loc, []); _} ] ) ], loc ->
  ...
  let body =
    [%expr
      Caml.Format.pp_open_box Debug_runtime.ppf 2;
      [%e arg_logs];
...
let debug_binding callback vb =
  ...
  let descr_loc, t_lident_loc_opt =
    ...
    | [%pat? ([%p? {ppat_desc=Ppat_var descr_loc; _} ] :
                [%t? {ptyp_desc=Ptyp_constr (t_lident_loc, []); _} ] ) ] ->
  ...
  | _, Some t_lident_loc ->
    ...
    let exp =
      [%expr
        Caml.Format.pp_open_box Debug_runtime.ppf 2;
        [%e log_preamble ~message:descr_loc.txt ~loc ()];
        let [%p result] = [%e callback vb.pvb_expr] in
        [%e log_value ~loc ~t_lident_loc ~descr_loc (pat2expr pat)];
        Caml.Format.pp_close_box Debug_runtime.ppf;
        [%e pat2expr result]] in
...
let debug_expander ~ctxt:_ payload =
  match payload with
  | { pexp_desc = Pexp_let (recflag, bindings, body); _ } ->
    (* This is the [let%debug ... in] use-case: do not debug the whole body. *)
     let bindings = List.map bindings ~f:(debug_binding (fun e -> traverse#expression e)) in
     {payload with pexp_desc=Pexp_let (recflag, bindings, body)}
  | expr -> expr

(*let debug_with_trace_expander ~ctxt payload =
  [%expr
    let backtrace = Caml.Printexc.get_callstack 5 in
    Caml.Printexc.print_raw_backtrace Stdio.stdout backtrace;
    [%e debug_expander ~ctxt payload]
  ]*)

let debug_this_expander ~ctxt:_ payload = traverse#expression payload
