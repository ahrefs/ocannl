...
type sync_properties =
  ...
  | Global
      (** This tensor is accessed directly in the global memory, we did not manage to optimize it. *)
[@@deriving sexp, equal, compare, variants]

type tensor = {
  hosted :  Node.ndarray option;
  ...
  local : string option;  (** A local array, if any. *)
...
let pp_index_axis ?provider_dim ppf =
  ...
  | Special_iterator Task_id -> Caml.Format.fprintf ppf "task_id"
  | Special_iterator Sample_num -> Caml.Format.fprintf ppf "sample_num"
...
let get_tensor
    { ctx; func; tensors; traced_store; device_to_host; host_to_device }
  ...
  Hashtbl.find_or_add tensors ptr ~default:(fun () ->
      ...
      let hosted = Option.value_exn @@ NodeUI.get_tensor ptr in
      let device_size_in_bytes = device_size * Node.precision_in_bytes hosted in
      let hosted = if host_size_in_bytes > 0 then Some hosted else None in
