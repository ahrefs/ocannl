...
**Windows verification placement**: GitHub-hosted Windows CI runs on its schedule, not per PR, and a merge never waits on the scheduled sweep. When a change needs Windows signal that matters (it fixes a Windows failure, or changes behavior only Windows exercises: line endings, float formatting in goldens, the mingw toolchain), dispatch `ci.yml` with `windows_only: true` and the full `expected_sha` and cite the run. When an issue's core development centers on Windows (iterating against Windows behavior rather than one check), stop and notify the user instead of working around it: `rog-nv-win` and `minix-amd-win` boot Ubuntu, and rebooting one into Windows is the user's to do. Under a wave coordinator, hand back to it naming the Windows need.
