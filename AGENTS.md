...
- **A GPU suite on a WSL2 box runs at `-j 2`** (`rog-nv-wsl`, `minix-amd-wsl`): their GPUs are reached through a `/dev/dxg` bridge whose VM-bus ring overflows at dune's default width, and the suite then comes back red in exactly the stanzas a real backend regression lands in. `tools/test-run.sh run` injects the cap and says so when it sees `/dev/dxg`, a GPU `OCANNL_BACKEND` and no `-j` of your own (an explicit `-j` is always honored, and a backend named only in an `ocannl_config` file cannot be seen from there — pass `-j 2` yourself); the cap lives in `tools/box-jobs.sh`, shared with `tools/sweep.sh`, and the refusal signature and recovery are the dxg bullet of `docs/agent-notes/build-and-test.md`
