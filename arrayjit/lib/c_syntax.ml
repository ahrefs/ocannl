...
module C_syntax (B : C_syntax_config) = struct
  ...
  let pp_device_read_ptr ctx tn ident_doc =
    ...
    if
      ...
      && Tn.Placements.is_materialized_force ctx.current_placements tn
           (Site "820:volatile-accumulation-read")
    then begin
  ...
  let is_local ctx tn =
    ...
    (not (Tn.Placements.is_virtual_force plc tn (Site "431:is-local-virtual-query")))
    && not (Tn.Placements.is_materialized_force plc tn (Site "432:is-local-materialized-query"))
  ...
  let zero_out_loop_redundant ctx tn =
    ...
    | Some node ->
        ...
        && (not (Tn.Placements.is_virtual_force plc tn (Site "337:zero-out-loop-redundant")))
    ...
    let ptr_params : (string * Tn.t) list =
      ...
      @@ Hashtbl.fold traced_store ~init:[] ~f:(fun ~key:tn ~data:_ acc ->
          let backend_info, is_param =
            ...
            if Tn.Placements.is_virtual_force plc tn (Site "334:kernel-param-virtual-query") then
              ("Virt", false)
            else if
              Tn.Placements.is_in_context_force placements tn (Site "46:kernel-param-in-context")
            then ("Ctx", true)
            else if Tn.Placements.is_materialized_force plc tn (Site "335:kernel-param-global") then
              ("Global", true)
    ...
    let local_decls =
      ...
      ^^ separate_map empty
           (fun (tn, node) ->
             ...
             if
               (not
                  (Tn.Placements.is_virtual_force plc tn (Site "333:local-decl-virtual-query")
                  || Tn.Placements.is_materialized_force plc tn
                       (Site "336:local-decl-materialized-query")))
