...
type backend_wrapper =
  | Wrapper : {
      ...
      context : ('buffer_ptr, ('buffer_ptr, 'dev, 'runner, 'event) BI.stream, 'optimize_ctx) BI.context;
    }
      -> backend_wrapper
...
let compile ctx comp bindings =
  ...
  let updated_wrapper = 
    Wrapper { wrapper with context = backend_routine.context }
  in
...
let run ctx routine =
  ...
  if not (Set.is_empty missing_inputs) then (
    let missing_names = 
      Set.to_list missing_inputs 
      |> List.map ~f:Tn.debug_name
      |> String.concat ~sep:", "
    in
...
let init_from_host_deprecated ctx tnode =
  ...
  let updated_wrapper = 
    Wrapper { wrapper with context = new_backend_context }
  in
...
let is_initialized ctx node =
  Set.mem ctx.initialized_nodes node
