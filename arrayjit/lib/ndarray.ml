...
(* export OCANNL_LOG_LEVEL_NDARRAY=9 to enable debugging into the log_files/ directory. *)
[%%global_debug_log_level_from_env_var "OCANNL_LOG_LEVEL_NDARRAY"]
...
type uint32_nd = (int32, Ops.int32_elt) bigarray (* Using int32 representation *)
type int64_nd = (int64, Ops.int64_elt) bigarray
type uint64_nd = (int64, Ops.int64_elt) bigarray (* Using int64 representation *)
...
let sexp_of_uint32_nd (arr : uint32_nd) = Sexp.Atom (big_ptr_to_string arr)
let sexp_of_int64_nd (arr : int64_nd) = Sexp.Atom (big_ptr_to_string arr)
let sexp_of_uint64_nd (arr : uint64_nd) = Sexp.Atom (big_ptr_to_string arr)
...
type t =
  ...
  | Uint32_nd of uint32_nd
  | Int64_nd of int64_nd
  | Uint64_nd of uint64_nd
...
let as_array (type ocaml elt_t) (prec : (ocaml, elt_t) Ops.precision)
  ...
  | Ops.Uint32 -> Uint32_nd arr
  | Ops.Int64 -> Int64_nd arr
  | Ops.Uint64 -> Uint64_nd arr
...
let precision_to_bigarray_kind (type ocaml elt_t) (prec : (ocaml, elt_t) Ops.precision) :
  ...
  | Ops.Uint32 -> Bigarray.Int32 (* Using int32 representation *)
  | Ops.Int64 -> Bigarray.Int64
  | Ops.Uint64 -> Bigarray.Int64 (* Using int64 representation *)
...
let get_prec = function
  ...
  | Uint32_nd _ -> Ops.uint32
  | Int64_nd _ -> Ops.int64
  | Uint64_nd _ -> Ops.uint64
...
let apply_with_prec { f } = function
  ...
  | Uint32_nd arr -> f Ops.Uint32 arr
  | Int64_nd arr -> f Ops.Int64 arr
  | Uint64_nd arr -> f Ops.Uint64 arr
...
let create_bigarray (type ocaml elt_t) (prec : (ocaml, elt_t) Ops.precision) ~dims ~padding :
  ...
  | Some (_, pad_value) -> (
      ...
      | Ops.Uint32 -> A.fill arr (Int32.of_float pad_value)
      | Ops.Int64 -> A.fill arr (Int64.of_float pad_value)
      | Ops.Uint64 -> A.fill arr (Int64.of_float pad_value)
