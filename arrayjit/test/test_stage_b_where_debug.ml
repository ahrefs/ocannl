(* task-9658aac9: non-vacuous regression for the Stage B unit-solve [Where] debug-OOB guard.
   ...
   Stage B unit-coefficient solving folds the producer read into [Ternop (Ops.Where, (range_cond,
   index_prec), (Get producer solved_idx, value_prec), (Get_local id, value_prec))] (see
   [Low_level.inline_computation]). For a non-matching kept-loop iteration [solved_idx] can be out
   of bounds; the runtime C ternary short-circuits, but debug value-logging ([C_syntax.debug_float])
   used to emit all three branch reads as unconditional printf arguments, dereferencing the producer
   out of bounds. In the real virtualized flow that [Where] sits inside a non-logged [Local_scope]
   ([log_set_locals:false]), so it never reaches [debug_float]; here we build the identical shapes
   in a *logged top-level [Set]* and run them through the actual backend codegen
   ([C_syntax.compile_proc]) -- a pure C-syntax path that emits the same C on every backend, so the
   test is backend-independent -- with debug value-logging enabled, then assert each
...
let mk ~id ~label ~dims =
  let tn =
    Tn.create (Tn.Default Ops.single) ~id ~label:[ label ]
      ...
      ()
  in
  Tn.update_memory_mode tn Tn.On_device 999;
  tn
...
let lt a b = LL.Binop (Ops.Cmplt, (LL.Embed_index a, index_prec), (LL.Embed_index b, index_prec))
...
let () =
  ...
  let inner_cond =
    LL.Binop
      (Ops.Cmplt, (LL.Get (condrd, [| Idx.Iterator t |]), Ops.single), (LL.Constant 1., Ops.single))
  in
  let inner_where =
    LL.Ternop
      ( Ops.Where,
  ...
  (* The inner condition's read [condrd3[t]] is gated by the OUTER guard: [(<outer_cond> ?
     condrd3[t] : 0)]. Without passing the enclosing guard into the condition this would be a bare
     unconditional printf argument (no [? condrd3[] form). Mutation: reverting the [debug_float]
     condition guard drops this. *)
