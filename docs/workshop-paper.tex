...
\paragraph{The approach in one example.}
A single line of OCANNL computes multi-head attention scores:
\begin{lstlisting}
q +* k "... s | h d; ... t | h d
        => ... s | t -> h" ["h"; "d"]
\end{lstlisting}
Every shape in OCANNL is partitioned into three \emph{axis kinds}---batch,
input, and output---written \spec{batch | input -> output}; the einsum spec
above gives that partition for each of the two operands and the result. The
ellipsis \spec{...} is a \emph{row variable}: it stands for any number of
batch axes, so the same code works whether the input has one batch dimension
or three. The labels \spec{h} and \spec{d} (heads, key dimension) are matched
positionally across the operands, and \code{["h"; "d"]} \emph{captures} their
inferred sizes as ordinary OCaml values, usable for scaling or for setting
the head count. The result spec \spec{... s | t -> h} places the key
positions \spec{t} on the output side and heads \spec{h} on the input side,
so the subsequent softmax---which reduces over output axes---naturally
reduces over \spec{t} while leaving \spec{h} alone. None of the projection
matrices feeding \spec{q}, \spec{k}, \spec{v} carry a dimension annotation:
all of their shapes are inferred. A standard einsum (\spec{bhsd,bhtd->bhst})
would commit to an exact axis count, check nothing until runtime, and offer
no capture or input/output structure.

\paragraph{How inference works.}
Shapes are not declared---they are \emph{inferred}, in the spirit of
Hindley--Milner type inference. As a model is constructed, each tensor
operation emits constraints relating the shapes of its operands and result:
equations from einsum operations, which demand exact matching, and
inequalities from pointwise and compose operations, which permit
broadcasting. A multi-stage solver resolves these constraints, progressing
from conservative propagation (run online, as expressions are built) to
increasingly aggressive closure of the remaining dimension and row variables.
Crucially, this all happens at model construction time, before any data flows
through the network, so a shape error surfaces when the offending line is
elaborated---not on the first forward pass. Each constraint carries a
\emph{provenance} record naming the einsum spec or operation that produced
it, so when unification fails the error message can point back to the
source-level specification rather than to an anonymous internal shape.

\paragraph{Contributions and outline.}
This paper presents the design of OCANNL's shape system and its inference
algorithm. Section~\ref{sec:shapes} describes the three axis kinds, the
generalized einsum notation, row variables, and optional dimension units.
Section~\ref{sec:inference} gives the constraint language and the multi-stage
solver that resolves shapes before execution, with provenance-tracked error
messages. Section~\ref{sec:ocaml} covers the PPX integration, and
Section~\ref{sec:examples} works through attention, convolution, and layer
normalization drawn from running OCANNL code. Readers satisfied by the
example above may treat the remainder as elaboration: the essential claim is
that generalized einsum plus row variables plus constraint inference yields
notation that is at once more concise, more general, and safer than the
status quo.
...
\paragraph{Suggested, not enforced.}
The axis kinds are a convention, not a type discipline: einsum specs may move
an axis from one kind to another (the attention example,
Section~\ref{sec:attention}, does exactly this). They are nonetheless not
redundant. Because each kind is a separate row, a shape admits \emph{three}
row variables instead of one, letting a specification leave one kind
polymorphic while constraining another---as in \spec{... | t -> ...}.
Collapsing the kinds into a single row would make every \spec{...}
all-or-nothing, losing the polymorphism the principle of least commitment
relies on (Section~\ref{sec:row-vars}).
...
Einsum specs can capture dimension variables for use in the surrounding
computation:
\begin{lstlisting}
let scores =
  q +* "... s | h d; ... t | h d
...
(\code{dim d} retrieves the integer dimension value). The capture bindings
are lifted to the nearest \emph{discharge point}---the top of the enclosing
function body, or the syntax-extension entry point if there is no enclosing
function definition---so they are in scope throughout that body, not merely
after the einsum expression.
...
variables to their greatest lower bounds or to ``no further axes'' at later
stages.
...
\item \textbf{Row variable compatibility}: row variables represent
  unknown numbers of axes. Named-axis systems cannot express ``some
  unknown axes with unknown names'' without reverting to positional
  structure. OCANNL's positional axes compose naturally with row
  variables.
\item \textbf{Conciseness}: the same matrix multiplication is
  \spec{"i,j;j,k=>i,k"} in OCANNL versus
  \texttt{(A[i,k]*B[k,j]).sum(k).order(i,j)} in torchdim---positional
  einsum is more compact for contraction-heavy patterns.
\end{enumerate}
...
dim_var (@$\mapsto$@) Solved(dim) | Bounds(glb, constraints, ordering)
row_var (@$\mapsto$@) Solved(row) | Bounds(glb, constraints, ordering)
\end{lstlisting}
...
\item \textbf{Stage~3}: Close row variables in \emph{terminal} (leaf) shapes
  to their greatest lower bounds (GLBs). Terminal shapes are closed first
...
\item \textbf{Stage~4}: Close terminal dimension variables with multiple
  upper bounds to their GLBs. Resolve \code{Total\_elems} constraints where
...
Three approaches to named dimensions have been explored.
Named tensor notation~\cite{chiang2021named} formalizes attaching
permanent string labels to axes, enabling type-like checking of
dimension alignment.
DumPy~\cite{dynomight2025dumpy} applies temporary string labels at
operation sites and compiles to \texttt{jax.vmap}, achieving
near-loop clarity without implicit broadcasting---external labels
are discarded after each operation, validating OCANNL's approach of
using temporary pseudo-labels in einsum specifications.
Torchdim~\cite{devito2024torchdim} introduces first-class dimension
objects with three rules~\cite{devito2024torchdim}: implicit batching
over the union of dimensions, dimensions as dimension specifiers, and
dimensions as index-range tensors. The third rule enables
index-component operations (e.g., upper-triangular masks) that are
complementary to einsum-based computation and could be adopted
orthogonally. Torchdim has been upstreamed into PyTorch as
\texttt{functorch.dim}~\cite{ansel2024pytorch2}.

All three named-axis approaches face inference ambiguity when
multiple axes share the same semantic unit.
OCANNL's positional identification with optional units avoids this
while row variables provide the flexibility that named approaches
achieve through batching rules.
...
OCANNL's shape system is currently monomorphic: shape variables are
interpreted existentially, not universally. This is not a loss of
expressivity---tensor operations are ordinary OCaml functions, and inference
constraints are built along OCaml tensor values, so re-running the
constructing code yields fresh variables. It is a loss of \emph{efficiency},
which can be substantial: a tensor-producing function called many times with
the same shapes redundantly re-derives, and shares, the same constraints
unless the user reconstructs it per call. The unit-parameter pattern
(\code{let layer = make () in ...}) makes this reconstruction explicit,
creating fresh variables per call. True shape polymorphism would abstract
over fresh variables when forming tensor functions and freshen them at
application sites---essentially an abstract interpretation mechanism---so
that a function need be constructed only once. This is planned for a future
version.
...
unit-parameter workarounds and enable shape-polymorphic libraries. It would
also surface some shape errors earlier: a function's internal constraints
could be checked once, at the function's own construction, rather than only
when its results are eventually lowered at a call site.
...
\bibitem{dynomight2025dumpy}
Dynomight.
\newblock {DumPy}: {NumPy} except it's {OK} if you're dum.
\newblock \url{https://dynomight.net/dumpy/}, 2025.

\bibitem{devito2024torchdim}
Zachary DeVito.
\newblock Torchdim: First-class dimensions for {PyTorch}.
\newblock \url{https://github.com/facebookresearch/torchdim}, 2024.
\newblock Archived; upstreamed into PyTorch as \texttt{functorch.dim}.
