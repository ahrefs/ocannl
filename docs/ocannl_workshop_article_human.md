...
$$
with $\alpha,\beta$ leaves and $\gamma$ interior. The store is satisfiable, e.g. $\alpha=\beta=1_\emptyset,\gamma=3_b$. Leaf-downward closing commits the capped leaves to their saturated GLBs, $\alpha \mapsto 3_b$ and $\beta \mapsto 5_b$; re-solving then pins $\gamma$ below two distinct atoms and fails. This is a policy rejection, not unsatisfiability: satisfying the store requires relaxing at least one capped leaf upward to $1_\emptyset$, and the deterministic closing policy deliberately does not backtrack to larger leaf shapes. Both examples are pinned by `test/einsum/test_closing_order.ml`.
...
**Remark 6.7 (Deterministic closing, and what backtracking would buy). [order-independence in A.7; design notes for (ii)–(iv)]**

(i) *Dimension sort: order-independent, not complete.* With transitively-closed $\mathrm{glb}$ stores (Remark 6.3's discipline), dimension leaf closing is order-independent in value: every capped leaf commits to the saturated join of its lower bounds, uncapped constrained leaves are deferred until re-solving can pin them or they become guessable, and no emission order can recreate the partial-$\mathrm{glb}$ branch ruled out in Remark 6.3. The stronger no-spurious-failure claim is false by Remark 6.3's second example. **[A.7 gives the order-independence fragment and records the counterexample.]**
...
- *Directed policy repairs, not arbitrary search:* dimension leaf caps can need upward relaxation (part (i)'s counterexample: raise one of the capped leaves to $1_\emptyset$), and row holes can need downward repair. Holes range over an infinite value domain, so blind enumeration is hopeless; but a spurious $1_\emptyset$-fill is *directed* — in the flat dimension order, a failed recheck names the only viable repair (its ground side) — so "retry with the value from the failure" is a finite discipline, and deferring hole commitments past all rechecks removes those failures without broad search. What remains at holes is Prop. 6.5's intent-level non-principality (guarded by the parameter error), not a satisfiability gap. For capped dimension leaves, the finite repair space is the set of committed leaves named by the failed upper-bound fan-in; raising any one that breaks the atom clash suffices in the displayed example, but the deterministic policy intentionally truncates before that branch.
