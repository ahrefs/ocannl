...
**Restriction**: `~logic:"@"` (Compose) is only meaningful with multiplication (`*` / `mul`). Using it with `/` (`div`) or `**` (`pow`) is a compile-time error: those combinations read like matrix inverse or matrix power but produce neither. Use `~logic:"."` for pointwise division/power, or an explicit einsum string for a custom contraction.
