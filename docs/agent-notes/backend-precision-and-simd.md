...
- **The `approximate` profile is the one word for the numerics-changing regime** (gh-ocannl-719):
  ...
  prints it in the report header as a JSON object, so a `narrow_compute_f32` or `cc_vector_bytes`
  in the operator's shell — or any key added after the gate was written — is visible in the
