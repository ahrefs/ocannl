...
let classify_point ~random_seed ~on_device ~inlining_cutoff ~num_devices ~batch ~backend_name precision () =
  ...
  let%track_sexp result : PrintBox_utils.table_row_spec =
    ...
    let result_jitted =
      Backend.jit_code sgd_update.context IDX.empty @@ Block_comment ("trivial infer", mlp_result.forward)
    in
...
let () =
  ...
  ignore
  @@ classify_point ~random_seed:3 ~on_device:true ~inlining_cutoff:3 ~num_devices:8 ~batch:8
...
let benchmarks =
  List.concat_map [ (* 0; 3; 5 *) 3 ] ~f:(fun inlining_cutoff ->
      List.concat_map [ 1; (* 2; 4; 8; 10; *) 16 (* ; 20 *) ] ~f:(fun num_devices ->
          List.concat_map [ (* 1; 8; *) 32; 64; 128 ] ~f:(fun batch ->
              List.concat_map [ 0; 1; 2 (* ; 3; 4 *) ] ~f:(fun random_seed ->
                  List.concat_map [ "gccjit" (* *; "cuda" *) ] ~f:(fun backend_name ->
                      [
                        classify_point ~random_seed ~on_device:true ~inlining_cutoff ~num_devices ~batch
...
let fixed_seed_search random_seed =
  classify_point ~random_seed ~on_device:true ~inlining_cutoff:3 ~num_devices:1 ~batch:20 ~backend_name:"cuda"
    CDSL.single ()
